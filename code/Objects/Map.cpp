...
Map::Map(Context &context)
  ...
  mLvlContext.playerInfo.backpack.setPosition(
      {(window_size.x * 8 / 9) - 100, 600});
  mLvlContext.playerInfo.equipment.setPosition(
      {(window_size.x * 8 / 9) - 20, 200});
}
...
void Map::update(sf::Time dt) {
  ...
  Utility::safeCasting<Text>(mGui->at("timer").get())
      ->setString("Time: " + Utility::timeToString(
                                 (context.statistics.get(Statistics::TimePlay) -
                                  mStartTime) /
                                 1000));
...
bool Map::isPlayerAlive() const { return mLevel->isPlayerAlive(); }
