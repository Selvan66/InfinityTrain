...
bool GraphicsSettingState::update(sf::Time dt)
{
    ...
    if (State::getGuiComponent<Checkbox>("FullscreenBox").isSelected())
        State::getGuiComponent<TextSlider>("ResolutionSlider").setCurrentText("1920x1080");
...
void GraphicsSettingState::applyGuiFunctions()
{
    ...
    State::getGuiComponent<TextSlider>("ResolutionSlider").setCurrentText(currentResolution);
    State::getGuiComponent<Checkbox>("FullscreenBox").setSelection(mSaveFullscreen);
    ...
    {
        std::string applyResolution = State::getGuiComponent<TextSlider>("ResolutionSlider").getCurrentText();
        ...
        context.settings.set<bool>(State::getGuiComponent<Checkbox>("FullscreenBox").isSelected(), "Graphics", "Fullscreen");
        context.applyGraphicSettings();
    });
    ...
    State::getGuiComponent<TextButton>("ApplySaveButton").setCallback([&]()
    {
        ...
        mSaveFullscreen = State::getGuiComponent<Checkbox>("FullscreenBox").isSelected();
        context.settings.set<std::pair<int, int>>(mSaveResolution, "Graphics", "Resolution");
        context.settings.set<bool>(mSaveFullscreen, "Graphics", "Fullscreen");
        context.applyGraphicSettings();
    });
