...
size_t
SceneNode::getChildrenSize() const {
...
void SceneNode::onCommand(
  const Command& command, sf::Time dt) {
...
void SceneNode::checkNodeCollision(
  ...
  if (this != &node &&
      Utility::collision(*this, node) &&
      !isDestroyed() &&
      !node.isDestroyed())
    collisionPair.insert(
      std::minmax(this, &node));

  for (auto& child : mChildren)
    child->checkNodeCollision(
      node, collisionPair);
}
...
void SceneNode::checkSceneCollision(
  ...
  std::set<Pair>& collisionPair) {
  checkNodeCollision(sceneGraph,
                     collisionPair);

  for (auto& child :
       sceneGraph.mChildren)
    checkSceneCollision(*child,
                        collisionPair);
}
...
void SceneNode::removeObjects() {
  auto wreckfieldBegin = std::remove_if(
    mChildren.begin(), mChildren.end(),
    std::mem_fn(
      &SceneNode::isMarkedForRemoval));
  mChildren.erase(wreckfieldBegin,
                  mChildren.end());
  std::for_each(
    mChildren.begin(), mChildren.end(),
    std::mem_fn(
      &SceneNode::removeObjects));
}
