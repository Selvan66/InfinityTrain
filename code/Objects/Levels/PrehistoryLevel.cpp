...
PrehistoryLevel::PrehistoryLevel(
  LvlContext& lvlContext)
  : Level(lvlContext) {
  buildScene();
}

void PrehistoryLevel::update(
  sf::Time dt) {
  Level::update(dt);
...
LevelID::ID
PrehistoryLevel::nextLevel() const {
  if (mDoor->isInteract())
    return LevelID::FirstLevel;
  return LevelID::None;
}

void PrehistoryLevel::
  buildBackground() {
  auto& context =
...
void PrehistoryLevel::buildFloor() {
  ...
  std::unique_ptr<Potion> test(
    new Potion(context, 50));
  test->setPosition({600, 500});
  floorLayer->attachChild(
    std::move(test));

  std::unique_ptr<Money> test2(
    new Money(context, 5));
  test2->setPosition({1000, 300});
  floorLayer->attachChild(
    std::move(test2));

  std::unique_ptr<Heart> test3(
    new Heart(context, 20));
  test3->setPosition({1200, 400});
  floorLayer->attachChild(
    std::move(test3));

  std::unique_ptr<Armor> test4(
    new Armor(context,
              Armor::HeavyHelmet));
  ...
  std::unique_ptr<Weapon> test8(
    new Weapon(context, Weapon::Sword));
  test8->setPosition({700, 900});
  floorLayer->attachChild(
    std::move(test8));

  std::unique_ptr<Weapon> test9(
    new Weapon(context, Weapon::Bow));
  test9->setPosition({700, 700});
  floorLayer->attachChild(
    std::move(test9));

  std::unique_ptr<Weapon> test10(
    new Weapon(context,
               Weapon::Bazooka));
  std::unique_ptr<Buyable> test11(
    new Buyable(context, 25,
                std::move(test10)));
  test11->setPosition({700, 600});
  floorLayer->attachChild(
    std::move(test11));

  std::unique_ptr<Weapon> test12(
    new Weapon(context, Weapon::Knife));
  test12->setPosition({700, 500});
  floorLayer->attachChild(
    std::move(test12));

  std::unique_ptr<Special> test13(
    new Special(context,
                Special::PortalGun));
