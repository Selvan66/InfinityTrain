...
SoundPlayer::SoundPlayer()
  : mSoundBuffer(), mSounds(),
    ListenerZ(300.f), Attenuation(8.f),
    MinDistance2D(200.f),
    MinDistance3D(std::sqrt(
      SoundPlayer::MinDistance2D *
        SoundPlayer::MinDistance2D +
      SoundPlayer::ListenerZ *
        SoundPlayer::ListenerZ)) {
  loadSounds();
  sf::Listener::setDirection(0.f, 0.f,
                             -1.f);
}
...
void SoundPlayer::play(SoundsID sound) {
  ...
  play(sound, sf::Vector2f(position.x,
                           position.y));
}
...
void SoundPlayer::setListenerPosition(
  sf::Vector2f position) {
  sf::Listener::setPosition(
    position.x, -position.y, ListenerZ);
}
...
void SoundPlayer::loadSounds() {
  mSoundBuffer.load(
    SoundsID::ButtonHover,
    "sounds/button_hover.wav");
  mSoundBuffer.load(
    SoundsID::ButtonClick,
