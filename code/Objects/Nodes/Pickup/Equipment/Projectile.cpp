...
Projectile::Projectile(Context &context, Type type, Category::Type category)
    : mType(type), mCategoryType(category),
      mAnimation(context.textures.get(projectiles[type].animation),
                 projectiles[type].animationRect),
      mTargetDirection(), mClosedEnemy(100000.f), mAttackCommand(),
      mFindCommand() {
  mAnimation.setFrameSize({32, 32});
  mAnimation.setNumFrames(projectiles[type].frameNum);
  mAnimation.setDuration(sf::seconds(0.5f));
  mAnimation.pause();

  if (category == Category::AlliedProjectile) {
    mAttackCommand.category = Category::Enemy;
    mFindCommand.category = Category::Enemy;
  } else if (category == Category::EnemyProjectile) {
    mAttackCommand.category = Category::Player;
    mFindCommand.category = Category::Player;
  } else {
    mAttackCommand.category = Category::None;
...
void Projectile::setDirection(sf::Vector2f pos) {
...
void Projectile::updateCurrent(sf::Time dt, CommandQueue &commands) {
  ...
  sf::Vector2f newVelocity = Utility::unitVector(
      approachRate * dt.asSeconds() * mTargetDirection + Entity::getVelocity());
