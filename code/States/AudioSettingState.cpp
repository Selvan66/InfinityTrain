...
void AudioSettingState::
  applyGuiFunctions() {
  auto& context = State::getContext();

  State::getGuiComponent<TextSlider>(
    "MusicSlider")
    .setCurrentText(std::to_string(
      static_cast<int>(mSaveMusic)));
  State::getGuiComponent<TextSlider>(
    "SoundSlider")
    .setCurrentText(std::to_string(
      static_cast<int>(mSaveSound)));
    ...
    .setCallback([&]() {
      float applyMusic = std::stof(
        State::getGuiComponent<
          TextSlider>("MusicSlider")
          .getCurrentText());
      float applySound = std::stof(
        State::getGuiComponent<
          TextSlider>("SoundSlider")
          .getCurrentText());
      context.settings.set(
        applyMusic, "Audio",
        "Music Volume");
