...
ControlSettingState::
  ControlSettingState(StateStack& stack,
                      Context& context)
  : State(stack, context),
    mActionBinding(
      context.settings
        .get<std::unordered_map<
          ...
          "Control")) // It's stupid but
                      // works
    ,
    mActionToChange(""), mChange(false),
    mChangeBackground(),
    mChangeText("PRESS ANY KEY",
                context.fonts.get(
                  FontsID::PixelFont),
                90) {
  const sf::Vector2f& window_size =
    context.window.getView().getSize();
  mChangeBackground.setFillColor(
    sf::Color(0, 0, 0, 150));
  mChangeBackground.setSize(
    context.window.getView().getSize());

  Utility::centerOrigin(mChangeText);
  mChangeText.setOutlineThickness(2.f);
  mChangeText.setOutlineColor(
    sf::Color(0, 0, 0));
  mChangeText.setPosition(
    sf::Vector2f(window_size.x / 2.f,
                 window_size.y / 2.f));
...
bool ControlSettingState::update(
  sf::Time dt) {
  if (!mChange)
    State::update(dt);
...
bool ControlSettingState::handleEvent(
...
void ControlSettingState::
  ...
  State::getGuiComponent<TextButton>(
    ...
    .setText(
      "Interact: " +
      mActionBinding["Interact"]);
  State::getGuiComponent<TextButton>(
    "PlayerSpecial")
    .setText("Special: " +
             mActionBinding["Special"]);
}

void ControlSettingState::
  changeActionBinding(std::string key) {
  auto found = std::find_if(
    mActionBinding.begin(),
    mActionBinding.end(),
    [&key](const auto& pair) {
      return pair.second == key;
    });
  if (found != mActionBinding.end())
    found->second = Utility::toString(
      sf::Keyboard::Unknown);
