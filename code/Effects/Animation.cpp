...
Animation::Animation()
  : mSprite(), mRect(), mFrameSize(),
    mNumFrames(), mCurrentFrame(0),
    mDuration(sf::Time::Zero),
    mElapsedTime(sf::Time::Zero),
    mRepeat(false), mReverse(false),
    mPaused(false) {}

Animation::Animation(
  const sf::Texture& texture)
  : mSprite(texture),
    mRect(0, 0, texture.getSize().x,
          texture.getSize().y),
    mFrameSize(), mNumFrames(),
    mCurrentFrame(0),
    mDuration(sf::Time::Zero),
    mElapsedTime(sf::Time::Zero),
    mRepeat(false), mReverse(false),
...
Animation::Animation(
  ...
  : mSprite(texture, rect), mRect(rect),
    mFrameSize(), mNumFrames(),
    mCurrentFrame(0),
    mDuration(sf::Time::Zero),
    mElapsedTime(sf::Time::Zero),
...
void Animation::setFrameSize(
  ...
  mSprite.setTextureRect(sf::IntRect(
    mRect.left, mRect.top, mFrameSize.x,
    mFrameSize.y));
  Utility::centerOrigin(mSprite);
}

void Animation::setNumFrames(
...
sf::FloatRect
Animation::getGlobalBounds() const {
...
sf::IntRect Animation::nextFrame(
  ...
  if (mReverse) {
    ...
    if (rect.left - mFrameSize.x <
        mRect.left) {
      rect.left =
        bounds.x - mFrameSize.x;
