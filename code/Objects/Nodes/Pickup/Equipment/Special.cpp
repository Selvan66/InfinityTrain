...
void initSpecialsActions(
  Context& context) {
  specials[Special::PortalGun]
    .action.category = Category::Player;
  specials[Special::PortalGun]
    .action.action =
...
Special::Special(Context& context,
  ...
  : Pickup(context), mType(type),
    mUse(false),
    mElapsed(specials[type].duration +
             sf::seconds(1)) {
  initSpecialsActions(
    context); // TODO: move this to a
              // better place
  Pickup::setTexture(
    specials[type].texture);
...
unsigned int
Special::getCategory() const {
  return Pickup::getCategory() |
         Category::Special;
}

std::string
Special::getDescription() const {
  std::stringstream ss;
