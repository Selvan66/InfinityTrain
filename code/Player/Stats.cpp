...
Stats::Stats(Context& context)
  : mStats() {
  ParserGui& parser =
    context.gui.get(GuiFileID::Stats);
...
bool Stats::updateStat(Type stat,
                       int value) {
  auto oldStat = mStats[stat];
...
bool Stats::updateStat(
  const std::unordered_map<Type, int>&
    stats) {
  bool changes = false;
  for (auto& stat : stats)
    changes = updateStat(stat.first,
                         stat.second)
                ? true
                : changes;
  return changes;
...
void Stats::updateStatsText() {
  Utility::safeCasting<Text>(
    ...
    ->setString(
      "Lives: " +
      std::to_string(mStats[Lives]));
  Utility::safeCasting<Text>(
    mGui->at("Armor").get())
    ->setString(
      "Armor: " +
      std::to_string(mStats[Armor]));
  Utility::safeCasting<Text>(
    mGui->at("Ammo").get())
    ->setString("Ammo: " +
                (mStats[Ammo] == INF
  ...
  Utility::safeCasting<Text>(
    ...
    ->setString(
      "Money: " +
      std::to_string(mStats[Money]));
  Utility::safeCasting<Text>(
    mGui->at("Attack").get())
    ->setString(
      "Attack: " +
      std::to_string(mStats[Attack]));
  Utility::safeCasting<Text>(
    mGui->at("Speed").get())
    ->setString(
      "Speed: " +
      std::to_string(mStats[Speed]));
}
...
void Stats::setStatWithRange(
  ...
  case Attack:
    mStats[stat] =
      std::max(0, value); // stat >= 0
    break;
  case Speed:
    mStats[stat] =
      std::max(25, value); // stat >= 25
    break;
  default:
    break;
  }
