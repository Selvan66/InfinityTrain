...
bool LoadingState::update(sf::Time dt) {
  ...
  if (mTexturesLoading.isFinished() &&
      mGuiFilesLoading.isFinished()) {
    loaded = true;
    requestStackPop();
    requestStackPush(
      StatesID::GameState);
...
void LoadingState::loadGameGuiFiles() {
  auto& context = State::getContext();
  context.gui.load(GuiFileID::Stats,
                   "gui/Stats.gui");
  context.gui.load(GuiFileID::Map,
                   "gui/Map.gui");
}

void LoadingState::loadGameTextures() {
  auto& context = State::getContext();
  context.textures.load(
    TexturesID::DefaultLevel,
    "image/defaultLevel.png");
  context.textures.load(
    TexturesID::PrehistoryLevel,
    "image/prehistoryLevel.png");
  context.textures.load(
    TexturesID::StartLevel,
    "image/StartLevel.png");
  context.textures.load(
  ...
  context.textures.load(
    ...
    "image/helmet.png");
  context.textures.load(
    TexturesID::ChestArmor,
    "image/armor.png");
  context.textures.load(
    TexturesID::HeavyBoots,
    "image/boot.png");
  context.textures.load(
    TexturesID::LightBoots,
    "image/boots.png");
  context.textures.load(
    TexturesID::Knife,
    "image/knife.png");
