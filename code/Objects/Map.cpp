...
Map::Map(Context& context)
  : mLvlContext{context,
    ...
    mLevel(nullptr),
    mStartTime(context.statistics.get(
      Statistics::TimePlay)),
    mGui() {
  registerLevels();
  mLevel =
    mFactories[LevelID::StartLevel]();

  sf::Vector2f window_size =
    sf::Vector2f(
      context.window.getView()
        .getSize());
  ParserGui parser =
    context.gui.get(GuiFileID::Map);
  parser.addConst("WINDOW_WIDTH",
                  window_size.x);
  parser.addConst("TEXT_HEIGHT", 70.f);
  mGui = parser.parse(context);

  mLvlContext.playerInfo.stats
    .setPosition(
      {window_size.x / 9, 300});
  mLvlContext.playerInfo.backpack
    .setPosition(
      {(window_size.x * 8 / 9) - 100,
       600});
  mLvlContext.playerInfo.equipment
    .setPosition(
      {(window_size.x * 8 / 9) - 20,
       200});
}

void Map::update(sf::Time dt) {
  auto& commands =
    mLevel->getCommandQueue();
  auto& context = mLvlContext.context;

  context.player.handleRealtimeInput(
    commands);
  mLvlContext.playerInfo.backpack
    .update();
  mLvlContext.playerInfo.equipment
...
void Map::registerLevels() {
  registerLevel<PrehistoryLevel>(
    LevelID::Prehistory);
  registerLevel<StartLevel>(
    LevelID::StartLevel);
  registerLevel<FirstLevel>(
    LevelID::FirstLevel);
}
