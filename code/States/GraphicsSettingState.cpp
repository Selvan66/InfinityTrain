...
bool GraphicsSettingState::update(
  sf::Time dt) {
  State::update(dt);

  if (State::getGuiComponent<Checkbox>(
        "FullscreenBox")
        .isSelected())
    State::getGuiComponent<TextSlider>(
      "ResolutionSlider")
...
void GraphicsSettingState::
  applyGuiFunctions() {
  auto& context = State::getContext();

  std::string currentResolution =
    std::to_string(
      mSaveResolution.first) +
    "x" +
    std::to_string(
      mSaveResolution.second);
  State::getGuiComponent<TextSlider>(
    "ResolutionSlider")
    .setCurrentText(currentResolution);
  State::getGuiComponent<Checkbox>(
    "FullscreenBox")
    ...
    .setCallback([&]() {
      context.settings
        .set<std::pair<int, int>>(
          mSaveResolution, "Graphics",
          "Resolution");
      context.settings.set<bool>(
        mSaveFullscreen, "Graphics",
        "Fullscreen");
  ...
  State::getGuiComponent<TextButton>(
    ...
    .setCallback([&]() {
      std::string applyResolution =
        State::getGuiComponent<
          TextSlider>(
          "ResolutionSlider")
          .getCurrentText();
      std::string width =
        applyResolution.substr(
          0, applyResolution.find('x'));
      std::string height =
        applyResolution.substr(
          applyResolution.find('x') + 1,
