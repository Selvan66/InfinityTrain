...
void State::loadGuiParser(
  GuiFileID guiFileID) {
  const sf::Vector2f& window_size =
    mContext.window.getView().getSize();

  ParserGui& parser =
    mContext.gui.get(guiFileID);
  parser.addConst("WINDOW_WIDTH",
                  window_size.x);
  parser.addConst("WINDOW_HEIGHT",
                  window_size.y);
  parser.addConst("TEXT_HEIGHT", 100.f);
  parser.addConst("TEXT_WIDTH", 270.f);
  parser.addConst("BUTTON_HEIGHT",
                  65.f);
