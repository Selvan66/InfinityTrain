  ...
  : mBackground(), mShow(false),
    mText("",
          context.fonts.get(
            FontsID::PixelFont),
          15),
    mObjectRect(), mContext(context) {}

void PopupLabel::setText(
...
void PopupLabel::setObjectRect(
  const sf::FloatRect& objectRect) {
  mObjectRect = objectRect;
}

void PopupLabel::handleEvent(
  const sf::Event& event) {
  if (event.type ==
      sf::Event::MouseMoved) {
    const auto& mouse = event.mouseMove;
    sf::Vector2f mousePos =
      mContext.window.mapPixelToCoords(
        sf::Vector2i(mouse.x, mouse.y));
    if (mObjectRect.contains(
          mousePos) &&
        !mText.getString().isEmpty()) {
      mShow = true;
...
void PopupLabel::setBackgoundRect() {
  ...
  mBackground.setFillColor(
    sf::Color(60, 60, 60));
  auto bounds =
    mBackground.getLocalBounds();
  mBackground.setOrigin(
