...
void Player::loadPlayerInput() {
  ...
  mKeyBinding[Utility::toKey(
    mContext.settings.get<std::string>(
      "Control", "Up"))] = MoveUp;
  mKeyBinding[Utility::toKey(
    mContext.settings.get<std::string>(
...
void Player::handleEvent(
  ...
  if (event.type ==
      sf::Event::KeyPressed) {
    auto found =
      mKeyBinding.find(event.key.code);
...
void Player::initializeActions() {
  ...
  mActionBinding[Fire].action =
    derivedAction<PlayerNode>(
      [](PlayerNode& p, sf::Time) {
        p.makeAction(PlayerNode::Fire);
      });
  mActionBinding[Interact].action =
    derivedAction<PlayerNode>(
      [](PlayerNode& p, sf::Time) {
        p.makeAction(
          PlayerNode::Interact);
      });
  mActionBinding[Special].action =
    derivedAction<PlayerNode>(
      [](PlayerNode& p, sf::Time) {
        p.makeAction(
          PlayerNode::Specials);
