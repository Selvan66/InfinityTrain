...
Pickup::Pickup(Context& context)
  : Interactable(), mContext(context),
    mSprite(), mPickedUp(false),
    mCommand() {
...
std::string
Pickup::getDescription() const {
  return "";
}

std::unordered_map<Stats::Type, int>
Pickup::getStats() const {
  return {{}};
}
...
void Pickup::setTexture(
  TexturesID texture) {
  mSprite.setTexture(
    mContext.textures.get(texture));
  Utility::centerOrigin(mSprite);
}

void Pickup::setCommand(
  Command command) {
  mCommand = command;
}
...
void Pickup::setLabel(
  const std::string& text) {
  std::unique_ptr<TextNode> textNode(
    new TextNode(mContext));
